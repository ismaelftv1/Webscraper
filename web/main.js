...
function RellenarTabla(datos = []) {
    ...
    for (let i = 0; i < datos.length; i++) {
        ...
        igic = precio * 1.07;
        total += igic;
        let descuento = datos[i].descuento;;

        //rellena la tabla con los datos de productos.json
        texto.innerHTML += `<tr><td>${datos[i].name}</td><td class="precio">${precio}€</td><td class="precio">${igic.toFixed(2)}€</td><td class="precio">${descuento}</td></tr>`;
    }
